...
/**
 * Cette fonction récupère les données d'un point de terminaison de l'API pour les travaux et les enregistre dans la console, tout en gérant les erreurs éventuelles.
...
async function fetchWorksApi() {
  ...
  /* Le bloc `catch` gère les erreurs qui peuvent survenir pendant l'exécution du bloc `try`.
   ...
   avec le message "erreur fetchWorksApi". Cela permet de s'assurer que le programme ne se plante pas et que les erreurs sont correctement gérées et rapportées.
...
/* `window.addEventListener("load", () => fetchWorksApi());` ajoute un écouteur d'événement à l'objet `window` qui écoute l'événement `load`.
l'objet `window` qui écoute l'événement `load`. Lorsque l'événement `load` est déclenché (c'est-à-dire lorsque le chargement de la page se termine), l'objet `fetchWorksApi' est activé.
page finit de se charger), la fonction `fetchWorksApi()` est appelée. Cela garantit que les données de l'API sont
données de l'API ne sont récupérées et enregistrées dans la console qu'une fois que la page a fini de se charger, ce qui permet d'éviter les problèmes potentiels liés à la synchronisation des données de l'API.
